...
module.exports=async function(req,res){
  ...
  try{
    ...
    for(const row of requested){const p=catalog.find(x=>String(x.id)===String(row.id));const qty=Math.floor(Number(row.quantity));if(!p||!p.active||ids.has(String(p.id))||qty<1||qty>100)throw Object.assign(new Error('Um produto do carrinho é inválido ou indisponível.'),{status:400});ids.add(String(p.id));items.push({id:p.id,name:p.name,quantity:qty,value:round(p.price),cost:round(p.cost),stock:Math.max(0,Math.floor(Number(p.stock)||0)),description:p.description||'Produto SQM'});}
    ...
    try{
      ...
      const payment=await asaas('/payments',{method:'POST',body:JSON.stringify({customer:asaasCustomer.id,billingType:paymentMethod,value:total,dueDate:today(),description:`Pedido ${orderId} - SQM`,externalReference:orderId})});paymentId=payment.id;
