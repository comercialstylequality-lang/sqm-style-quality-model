...
module.exports=async function(req,res){
  ...
  try{
    ...
    if(req.method==='PATCH'){
      ...
      order.status=status; order.paymentStatus=['Pago','Processando','Enviado','Entregue'].includes(status)?'Pago':status==='Cancelado'?'Cancelado':'Pendente'; order.updatedAt=new Date().toISOString();
